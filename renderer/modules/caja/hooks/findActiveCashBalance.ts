...
export default async function findACashById(cashId: number): Promise<ICashBalance | null> {
    try {
      const cashBalance = await strapi.findOne(getCashBalanceKey(), cashId) as unknown as ICashBalance

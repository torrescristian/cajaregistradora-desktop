import { TICKET_STATUS, ITicketPayload, PAYMENT_TYPE } from '@/interfaces/ITicket';
...
export default function useCreateTicketMutation() {
  ...
  function calcNewCashAmount(ticket: ICreateTicketMutation) {
    ...
    const cashPayment = ticket.payments.find(payment => payment.type === PAYMENT_TYPE.CASH)
  ...
  return useMutation(async (ticket: ICreateTicketMutation) => {
    ...
    const sum = ticket.payments.reduce((acc, curr) => acc + Number(curr.amount), 0);
    if (sum !== ticket.totalPrice) {
      throw new Error(`No se está cobrando correctamente (total: ${ticket.totalPrice}, cobrando: ${sum})`);
    }

...
export default function useCancelTicketMutation() {
  ...
  return useMutation(async ({ ticketId, orderId, amountTicket,cashBalance, returnType}: IProps) => {
    ...
    const updateTicketResult = await strapi.update(getTicketsQueryKey(), ticketId, {
      status: TICKET_STATUS.REFUNDED,
    });
    const returnMoney = await strapi.update(getCashBalanceKey(), cashBalance.id!  , {
      newCashAmount: returnType === 'cash' ? Math.max(cashBalance.newCashAmount - amountTicket,0) : cashBalance.newCashAmount,

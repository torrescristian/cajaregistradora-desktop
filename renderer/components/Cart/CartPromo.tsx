            ...
            {promosItems.map(({ promo, selectedVariants },index) => (
                <div key={promo.id!} className='flex flex-col border-2 p-5 gap-3'>
                    <div className="flex flex-row items-center gap-4">
                        ...
                        <button className="btn btn-error text-neutral-content" onClick={handleClickPromoRemove(index)}>
                            <TrashIcon className="w-5 h-5" />
                        </button>
                    </div>
                    {promo.categories.map(({ category, quantity }) => (
                        <div className='flex flex-col p-2' key={category.id!}>
                            <p className='list-item'>{category.name} <span className='badge-primary badge-outline'>x{quantity}</span></p>
                        </div>
                    ))
                    }
                    <div className='p-2'>{promo.variants.map(({ variant, quantity }) => (
                        <p className='list-item'>{variant.product} - {variant.name} <span className='badge-primary badge-outline'>x{quantity}</span></p>
                    ))}
                    </div>

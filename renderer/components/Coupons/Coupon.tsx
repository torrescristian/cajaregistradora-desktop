...
export default function Coupon({ coupon }: IProps) {
  ...
  return (
    <section className="flex flex-col border-2 border-primary-content shrink-0 p-3 gap-4">
      ...
      <button className='btn btn-error' onClick={handleCancelCupon}><TrashIcon className='h-5 w-5'/></button>  
      </div>
      <DataItem label="Descuento:" value={formatPrice(coupon.discount.amount)} />
      <DataItem
        label="Vencimiento:"
        value={parseDateToArgentinianFormat(coupon.dueDate)}
      />
      <DataItem label='tipo de descuento:' value={convertDiscount(coupon.discount)}/>
      <DataItem label="Limite:" value={formatPrice(coupon.maxAmount)} />
      
      <DataItem
        label="Producto:"
        value={coupon.variant ? `${convertToEmoji(coupon.variant.product.type)} ${
